...
export class CreatePlayerScoreDto {
    ...
    @Validate(IsType, [String, CreatePlayerDto], {message: "Invalid Player. Player must be either the name of an existing player name or the corresponding uuid, or a new player. "})
    @Type((r) => {
    ...
    @Type((r) => {
        return typeof r.object[r.property] === "string" ? String : LapWithoutTrackAndPlayer
    })
    @Expose()
