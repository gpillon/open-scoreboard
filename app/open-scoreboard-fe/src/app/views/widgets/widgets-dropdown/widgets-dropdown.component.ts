...
import {LapService} from "../../../api/services/lap.service";
import {Observable} from "rxjs";
import {map} from "rxjs/operators";
import {TrackService} from 'src/app/api/services/track.service';
...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
  ...
  lapCount: Observable<number> = this.lapService.lapControllerCount().pipe(map(c => c.count))
  trackCount: Observable<number> = this.trackService.trackControllerCount().pipe(map(c => c.count))
  playerCount: Observable<number> = this.playerService.playerControllerCount().pipe(map(c => c.count))
  gameCount: Observable<number> = this.gameService.gameControllerCount().pipe(map(c => c.count))
